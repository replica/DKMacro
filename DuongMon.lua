...
function MacroFunctions.DuongMonKVQ()
  ...
  if bEnable==true and MacroFunctions.StopAction()<=1 and MacroFunctions.CheckMoveState(p,"stand|float|entrap|walk|run|jump") and not MacroFunctions.KTCombo.bEnable then
    ...
    if energy>=(x*2) and (MacroFunctions.CheckBuff(p,3276,1,0,0) or MacroFunctions.tmtReady) and (MacroFunctions.CheckBuff(p,3468,1,0,0) or not MacroFunctions.IsSkillCD(3094)) and (not bHuyenDieu or MacroFunctions.CheckBuff(p,3468,1,0,0)) then MacroFunctions.use({3096},y) end
    ...
    if RawTargetHP>=HPLimit and energy>=(x*3) and not MacroFunctions.CheckBuff(T,3712,1,0,0) and p.GetSkillLevel(5703)==1 and (p.GetSkillLevel(6891)==0 or not bHuyenDieu or MacroFunctions.CheckBuff(p,3468,1,0,0)) then MacroFunctions.use({3098},y) end
    --Đánh Hóa Huyết Tiêu lên target nếu chưa tồn tại
    if RawTargetHP>=HPLimit and energy>=(x*2) and not MacroFunctions.CheckBuff(T,2237,1,0,0) and (not bHuyenDieu or MacroFunctions.CheckBuff(p,3468,1,0,0)) and MacroFunctions.dwLastSkillIDCasted~=3087 then MacroFunctions.use({3087},y) end
    --Khi đủ thần cơ dùng Trục Tinh Tiễn, Liên Hoàn Nỏ, Ngự Nguyên Tiễn, Khổng Tước Linh, BVLHC, Đoạt Phách Tiễn, Liệt Thạch Nỏ
    if p.GetSkillLevel(6891)==1 then
      if (not bHuyenDieu or MacroFunctions.CheckBuff(p,3468,1,0,0)) and (energy<60 or MacroFunctions.CheckBuff(p,3468,1,0,0)) then MacroFunctions.use({3101},y) end
    else
      if (not bHuyenDieu or MacroFunctions.CheckBuff(p,3468,1,0,0)) and ((MacroFunctions.CheckBuff(T,2237,1,0,0) and (MacroFunctions.CheckBuff(T,3712,1,0,0) or MacroFunctions.GetSkillCD(3098)>1.5) and p.GetSkillLevel(6776)==1) or energy<=60) then MacroFunctions.use({3101},y) end
    end
    ...
    if energy>=(x*1.5) and (energy<=55 or MacroFunctions.CheckMoveState(p,"walk|run|jump")) and (not bHuyenDieu or MacroFunctions.CheckBuff(p,3468,1,0,0)) then MacroFunctions.use({3088},y) end
    ...
    if energy>=(x*3) and MacroFunctions.CheckMoveState(p,"stand|float|entrap") and p.GetSkillLevel(6891)==0 then MacroFunctions.use({3095},y) end
...
function MacroFunctions.DuongMonTLND()
  ...
  if bEnable==true and MacroFunctions.StopAction()<=1 and MacroFunctions.CheckMoveState(p,"stand|float|entrap|walk|run|jump") and not MacroFunctions.KTCombo.bEnable then
    ...
    if MacroOptions.autoTCB==1 then
      ...
      if (MacroFunctions.CheckBuff(T,3253,MacroOptions.stackNumPheTam,4,0) or not (MacroFunctions.CheckSkillRecipe(3087,825) or MacroFunctions.CheckSkillRecipe(3087,826))) and ((MacroFunctions.GetATSCNum()==3 and MacroFunctions.GetSkillCD(3111)<1) or MacroFunctions.GetATSCNum()==3) then
        if MacroOptions.autoUseWeapon and MacroFunctions.IsSkillCD(3094) then MacroFunctions.UseEquippedItem(EQUIPMENT_INVENTORY.MELEE_WEAPON) end
        if MacroOptions.autoUseAmulet and MacroFunctions.IsSkillCD(3094) then MacroFunctions.UseEquippedItem(EQUIPMENT_INVENTORY.AMULET) end
        if MacroOptions.autoUsePendant and MacroFunctions.IsSkillCD(3094) then MacroFunctions.UseEquippedItem(EQUIPMENT_INVENTORY.PENDANT) end
      end
    ...
    if MacroOptions.autoTCB==1 then
      if MacroOptions.autoTapTrungQPTC==1 then
        if MacroFunctions.GetSkillCD(3110)>10 and MacroFunctions.GetSkillCD(3110)<115 then
          if MacroOptions.autoTapTrungTLND and (MacroFunctions.CheckBuff(T,3253,MacroOptions.stackNumPheTam,4,0) or not (MacroFunctions.CheckSkillRecipe(3087,825) or MacroFunctions.CheckSkillRecipe(3087,826))) and (MacroFunctions.GetSkillCD(3093)<=8 or MacroFunctions.GetSkillCD(6444)<=8) and ((MacroFunctions.GetATSCNum()==3 and MacroFunctions.GetSkillCD(3111)<1) or MacroFunctions.GetATSCNum()==3) then MacroFunctions.use({3094},3,500) end
        else
    ...
    if not (RawTargetHP>=HPLimit and ((MacroOptions.autoTCB==1 and distance<25) or (MacroOptions.autoTCB==2 and distance<9.5)) and (MacroFunctions.GetTCBStatus()==-1 or (MacroFunctions.TCBTimeLeft<=TCBTimeLeft and MacroFunctions.TCBTimeLeft>=0)) and MacroFunctions.GetSkillCD(3109)==MacroFunctions.GetSkillCD(3087) and MacroFunctions.dwPreparingSkillID~=3109 and MacroFunctions.dwLastSkillIDCasted~=3109 and not MacroFunctions.CheckBuff(p,3316,1,0,0) and not MacroFunctions.CheckBuff(p,3468,1,0,0)) then
      ...
      --Đặt Ám Tàng Sát Cơ
      if (MacroFunctions.CheckMoveState(T,"stand|float") or MacroOptions.allowTargetMove) and MacroOptions.toggleAttackMode~=2 and MacroFunctions.GetATSCNum()<=2 and ((energy>=MacroOptions.toggleATSCMode or (MacroFunctions.GetSkillCD(3110)<=8 and energy>=25 and MacroOptions.autoTCB>0 and MacroOptions.autoQuiPhuThanCong) or (MacroFunctions.GetSkillCD(3094)<=8 and energy>=25 and MacroOptions.autoTapTrungQPTC==1 and MacroOptions.autoTapTrungTLND) or MacroFunctions.CheckBuff(p,3468,1,0,0) or MacroFunctions.CheckBuff(p,3316,1,0,0)) and (not bHuyenDieu or MacroFunctions.CheckBuff(p,3468,1,0,0))) then MacroFunctions.use({3111},1,0) end
      --Nổ Ám Tàng Sát Cơ khi có Tập Trung hoặc QPTC
      if IsEnemy(p.dwID,T.dwID) and ((MacroFunctions.GetATSCNum()==3 and ((MacroFunctions.CheckBuff(p,3468,1,0,0) and MacroFunctions.GetSkillCD(3110)<114) or (MacroFunctions.CheckBuff(p,3316,1,0,0) and MacroFunctions.GetSkillCD(3094)<88) or (MacroFunctions.CheckBuff(p,3468,1,0,0) and MacroFunctions.CheckBuff(p,3316,1,0,0) and MacroFunctions.GetSkillCD(3094)>=88 and MacroFunctions.GetSkillCD(3110)>=114))) or (MacroFunctions.GetATSCNum()>=2 and not MacroFunctions.CheckBuff(p,3468,1,1.1,1)) or (MacroFunctions.GetATSCNum()>=2 and not MacroFunctions.CheckBuff(p,3316,1,1.1,1) and not MacroFunctions.CheckBuff(p,3468,1,0,0))) then MacroFunctions.use({3357},2) end
