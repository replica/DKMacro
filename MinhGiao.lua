...
function MacroFunctions.MinhGiaoPATQ(arg)
  ...
  if bEnable==true and MacroFunctions.StopAction()<=1 and MacroFunctions.CheckMoveState(p,"stand|float|entrap|walk|run|jump") and not MacroFunctions.KTCombo.bEnable then
    ...
    for i = 1,p.GetBuffCount() do
      ...
      if dwID==4937 and nLevel==2 and nStackNum==2 and (FS==1 or FM==1) then
        NNDH=true
      end
      if (dwID==4937 and nLevel>0) or FS==1 or FM==1 then
        NLNH=false
      end
    end
    ...
    if p.bFightState or not MacroFunctions.IsSkillCD(3974) or MacroOptions.autoAmTranDiTan==0 or FS==1 or FM==1 then
      ...
      if (FS==1 or FM==1) and (not MacroFunctions.CheckBuff(p,4937,7,0,0) or MacroFunctions.CheckBuff(p,4930,1,0,0)) and not MacroFunctions.CheckBuff(p,4052,1,0,0,1) then MacroFunctions.use(TinhThe,2,500) end
      ...
      if not arg then
        ...
        if FS==1 and MacroFunctions.CheckBuff(p,4937,7,0,0) then MacroFunctions.use({3962},2,500) end
        if FM==1 and MacroFunctions.CheckBuff(p,4937,7,0,0) then MacroFunctions.use({3959},2,500) end
      ...
      elseif arg==1 then
        ...
        if (FS==1 or FM==1) and MacroFunctions.CheckBuff(p,4937,7,0,0) then MacroFunctions.use({3962},2,500) end
        --Đánh thêm UNL (sau KDDS)
        if NhatLinh<7000 and NguyetHon>7600 and not MacroFunctions.CheckBuff(p,4937,1,0,0) then MacroFunctions.use({3959},2,500) end
        --Đánh thêm Ngân Nguyệt Trảm
        if MacroOptions.autoNNLN and (not MacroFunctions.CheckBuff(p,4937,1,0,0,1) or (MacroFunctions.CheckBuff(p,4937,3,0,0,1) and not MacroFunctions.CheckBuff(p,4937,7,0,0,1)) or MacroFunctions.CheckBuff(p,4937,2,0,0,2)) and not MacroFunctions.CheckBuff(p,4056,1,0,0) and not MacroFunctions.CheckBuff(T,4202,1,0,0) then MacroFunctions.use({3960},2,500) end
        ...
        if not MacroFunctions.CheckBuff(p,4056,1,0,0) then
          if not MacroFunctions.CheckBuff(p,4937,1,0,0,1) or (MacroFunctions.CheckBuff(p,4937,3,0,0,1) and not MacroFunctions.CheckBuff(p,4937,7,0,0,1)) or MacroFunctions.CheckBuff(p,4937,2,0,0,2) then MacroFunctions.use({3963},2,500) end
        else
      ...
      elseif arg==2 then
        ...
        if (FS==1 or FM==1) and MacroFunctions.CheckBuff(p,4937,7,0,0) then MacroFunctions.use({3959},2,500) end
        --Đánh thêm XNL (sau KDDS)
        if NguyetHon<7000 and NhatLinh>7550 and not MacroFunctions.CheckBuff(p,4937,1,0,0) then MacroFunctions.use({3962},2,500) end
        --Đánh thêm Liệt Nhật Trảm
        if MacroOptions.autoNNLN and (not MacroFunctions.CheckBuff(p,4937,1,0,0,1) or (MacroFunctions.CheckBuff(p,4937,3,0,0,1) and not MacroFunctions.CheckBuff(p,4937,7,0,0,1)) or MacroFunctions.CheckBuff(p,4937,2,0,0,2)) and not MacroFunctions.CheckBuff(p,4056,1,0,0) and not MacroFunctions.CheckBuff(T,4418,1,0,0) then MacroFunctions.use({3963},2,500) end
        ...
        --Combo chính
        if not MacroFunctions.CheckBuff(p,4056,1,0,0) then
          if not MacroFunctions.CheckBuff(p,4937,1,0,0,1) or (MacroFunctions.CheckBuff(p,4937,3,0,0,1) and not MacroFunctions.CheckBuff(p,4937,7,0,0,1)) or MacroFunctions.CheckBuff(p,4937,2,0,0,2) then MacroFunctions.use({3960},2,500) end
        else
